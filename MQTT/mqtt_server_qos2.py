    ...
    for file in os.listdir(path):
        with open(path+file, "rb") as f:
            ...
            for i in range(ftc[file]):
                ...
                result = publisher.publish(topic=topic_to_send_message, payload=file_read, qos=2)
                result.wait_for_publish()
        print("sent ", file, " ", ftc[file], " times")

    # Transmit a message indicating the end of the file transfer
    result = publisher.publish(topic=topic_to_send_message, payload="end", qos=2)
    result.wait_for_publish()
...
def run_mqtt_file_transfer_test():
    ...
    publisher.subscribe(topic_to_receive_message, qos=2)
